...
# --- GÜVENLİK ---
if "oturum_acik" not in st.session_state: st.session_state.oturum_acik = False
...
with st.sidebar:
    ...
    svg[data-testid="stIcon-ChevronDown"] {{ fill: #00ff00 !important; transform: scale(1.4) !important; }}
    .stTextInput input, .stNumberInput input, .stSelectbox div {{ background-color: {'#050505' if dark_mode else '#fff'} !important; color: {'#00ff00' if dark_mode else '#000'} !important; border: 1px solid #00ff00 !important; }}
    ...
    with st.expander("🏦 Banka & Sermaye", expanded=True):
        ...
        # DÜZELTİLDİ: ORAN MİLYONDA 375 OLARAK MÜHÜRLENDİ
        b_or = st.number_input("Komisyon (%)", value=0.000375, format="%.6f", step=0.000001)
        if st.button("Bankayı Mühürle"): db.banka_ekle(b_ad, b_or); st.rerun()
        s_tut = st.number_input("Sermaye +/-", value=0.0)
        if st.button("Sermayeyi Güncelle"): db.para_islem(s_tut); st.rerun()

    # --- TEK KUTU HİSSE GİRİŞİ (Bariyerler Kaldırıldı) ---
    ...
    if st.button("🚀 PORTFÖYE MÜHÜRLE", use_container_width=True):
        if h_kod and h_fiy > 0 and h_ban:
            db.islem_kaydet(h_kod, h_lot, h_fiy, "AL", h_ban); st.rerun()
    ...
    with st.expander("❌ Hatalı Kayıt Düzelt", expanded=False):
        ...
        if isl:
            ...
            if st.button("SİL"):
                if db.islem_sil(int(sec.split(":")[0])): st.rerun()
...
with tab1:
    ...
    for s, v in db.data["aktif_hisseler"].items():
        if v["lot"] > 0:
            ...
            deger = v["lot"] * g_fiyat
            toplam_v += deger
            g_data["Hisse"].append(s); g_data["Değer"].append(deger)
            karar = analiz.ai_katı_strateji(s, v["maliyet"], g_fiyat, db.data.get("zarar_havuzu", 0.0))
            p_list.append({"Hisse": s, "Lot": v["lot"], "Maliyet": f"{v['maliyet']:.4f}", "Güncel": f"{g_fiyat:.2f}", "AI Kararı": karar})

    c1, c2, c3, c4 = st.columns(4)
    # HATA GİDERİLDİ: Metod ismi database_manager ile senkronize edildi
    ...
    with ct:
        if p_list: st.dataframe(pd.DataFrame(p_list), use_container_width=True, hide_index=True)

with tab2:
    st.subheader("🔍 Şirket Röntgeni")
    b_hisse = st.selectbox("Hisse Seç", db.kayitli_hisseler())
    if b_hisse: st.write(analiz.bilanco_analiz(b_hisse))

with tab3:
    st.subheader("📰 Akıllı KAP")
    k_hisse = st.selectbox("Haber Seç", db.kayitli_hisseler())
    if k_hisse:
        kap = analiz.kap_yorumlari(k_hisse)
        st.chat_message("assistant", avatar="🤖").markdown(f"**Haber:** {kap['haber']}\n\n**AI Yorumu:** {kap['yorum']}")